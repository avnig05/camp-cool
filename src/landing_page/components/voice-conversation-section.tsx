...
// Decorative Elements (Optional, but can clean up the main return)
...
const QuestionItem: React.FC<QuestionItemProps> = ({ question, index, isActive, onClick }) => (
  <motion.div
    key={index}
  ...
  >
    <div className="flex items-start gap-3">
      <div
        className={`mt-1 ${isActive ? "text-white" : "text-[#4F9F86]"} transition-colors duration-300`}
      >
...
interface AiVoiceTechnologySectionProps {
  isPlaying: boolean;
  isClient: boolean;
  togglePlayback: () => void;
  getSineBasedHeight: (i: number, offset?: number) => number;
  getInitialHeight: (i: number) => number;
}

const AiVoiceTechnologySection: React.FC<AiVoiceTechnologySectionProps> = ({
  isPlaying,
  isClient,
  togglePlayback,
  getSineBasedHeight,
        ...
        <div className="h-10 flex items-center">
          {Array.from({ length: 30 }).map((_, i) => {
            const height = isPlaying && isClient
              ? getSineBasedHeight(i, Date.now() / 1000)
              : getInitialHeight(i)
            return (
              <motion.div
                key={i}
                ...
                animate={isClient && isPlaying ? { height: [height, height + 5, height] } : {}}
                transition={{
                  duration: 0.5,
...
const VoiceRecordingSectionDisplay: React.FC<VoiceRecordingSectionProps> = ({
  isRecording,
  isClient,
  toggleRecording,
  waveformData,
...
const VoiceConversationSection = () => {
  ...
  // Removed audioRef as it was not used after initial assignment
  ...
  const generateSeededRandom = useCallback((seed: number) => {
    let value = seed
    return function() {
  ...
  }, [isRecording, isClient, generateSeededRandom])
  ...
  const getInitialHeight = useCallback((i: number) => {
    return 3 + (i % 5)
  }, [])

  const getSineBasedHeight = useCallback((i: number, offset = 0): number => {
    return Math.sin(((i / 30) * Math.PI) + offset) * 20 + 5 // Increased amplitude for visibility
  }, [])
  ...
  return (
    <div className="relative">
      ...
      <div className="relative z-10 bg-gradient-to-br from-[#FFF9E3]/80 to-white rounded-3xl shadow-xl border border-[#4F9F86]/10 p-8 overflow-hidden">
        ...
        <AiVoiceTechnologySection
          isPlaying={isPlaying}
          isClient={isClient}
          togglePlayback={togglePlayback}
          getSineBasedHeight={getSineBasedHeight}
          getInitialHeight={getInitialHeight}
        />
        <VoiceRecordingSectionDisplay
